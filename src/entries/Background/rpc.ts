  ...
  browser.runtime.onMessage.addListener(
    (request, sender, sendResponse): any => {
      ...
      switch (request.type) {
        ...
        case BackgroundActiontype.run_dynamic_proof_request:
          return handleRunDynamicProofCSRequest(request);
        default:
          break;
      }
    }
...
async function handleConnect(request: BackgroundAction) {
  ...
  if (!connection) {
    ...
    const { popup, tab } = await openPopup(
      `connection-approval?origin=${encodeURIComponent(
        request.data.origin
      )}&favIconUrl=${encodeURIComponent(currentTab?.favIconUrl || "")}`,
      request.data.position.left,
      request.data.position.top
    );
...
//   const { popup, tab } = await openPopup(
//     `get-zap-approval?method=${method}&url=${url}&origin=${encodeURIComponent(
//       origin
//     )}&favIconUrl=${encodeURIComponent(currentTab?.favIconUrl || "")}`,
//     position.left,
//     position.top
...
//       filterMetadata ? `metadata=${JSON.stringify(filterMetadata)}&` : ""
//     }method=${filterMethod}&url=${filterUrl}&origin=${encodeURIComponent(
//       origin
//     )}&favIconUrl=${encodeURIComponent(currentTab?.favIconUrl || "")}`,
//     position.left,
//     position.top
...
//   const { popup, tab } = await openPopup(
//     `get-proof-approval?id=${id}&origin=${encodeURIComponent(
//       origin
//     )}&favIconUrl=${encodeURIComponent(currentTab?.favIconUrl || "")}`,
//     position.left,
//     position.top
...
//   const { popup, tab } = await openPopup(
//     `notarize-approval?config=${encodeURIComponent(
//       config
//     )}&origin=${encodeURIComponent(origin)}&favIconUrl=${encodeURIComponent(
...
//   const { popup, tab } = await openPopup(
//     `install-plugin-approval?${
//       metadata ? `metadata=${JSON.stringify(metadata)}&` : ""
//     }url=${url}&origin=${encodeURIComponent(
//       origin
//     )}&favIconUrl=${encodeURIComponent(currentTab?.favIconUrl || "")}`,
//     position.left,
//     position.top
...
//   const { popup, tab } = await openPopup(
//     `get-plugins-approval?${
//       filterMetadata ? `metadata=${JSON.stringify(filterMetadata)}&` : ""
//     }&filterOrigin=${filterOrigin}&url=${filterUrl}&origin=${encodeURIComponent(
//       origin
//     )}&favIconUrl=${encodeURIComponent(currentTab?.favIconUrl || "")}`,
//     position.left,
//     position.top
...
//   const { popup, tab } = await openPopup(
//     `run-plugin-approval?hash=${hash}&origin=${encodeURIComponent(
//       origin
//     )}&favIconUrl=${encodeURIComponent(currentTab?.favIconUrl || "")}`,
//     position.left,
//     position.top
...
async function handleRunDynamicProofCSRequest(request: BackgroundAction) {
  const [currentTab] = await browser.tabs.query({
    active: true,
    currentWindow: true,
  });
  ...
  const defer = deferredPromise();
  const { origin, position, type } = request.data;

  const originalTabId = currentTab.id;

  // const config = await getProofConfigByType(type);
  let isUserClose = true;

  // if (!config) {
  //   defer.reject(new Error("request type not found."));
  //   return defer.promise;
  // }

  const { popup, tab } = await openPopup(
    `dynamic-proof-approval?type=${type}&origin=${encodeURIComponent(
      origin
    )}&favIconUrl=${encodeURIComponent(currentTab?.favIconUrl || "")}`,
    position.left,
    position.top
  ...
  const onProofRequest = async (req: any) => {
    ...
    browser.runtime.onMessage.removeListener(onProofRequest);
  };

  const onMessage = async (req: BackgroundAction) => {
    if (req.type === BackgroundActiontype.run_dynamic_proof_response) {
      if (req.data) {
        console.log("run_dynamic_proof_response", req.data);
        browser.runtime.onMessage.addListener(onProofRequest);
      } else {
        defer.reject(new Error("user rejected."));
      }
  ...
  const onPopUpClose = (windowId: number) => {
    console.log("outside if of onPopClose");
    if (isUserClose && windowId === popup.id) {
      console.log("inside if of onPopClose");
      defer.reject(new Error("user rejected."));
      browser.windows.onRemoved.removeListener(onPopUpClose);
    }
  };
  ...
  console.log("Promise final no rpc", defer.promise);
