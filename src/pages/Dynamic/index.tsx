...
export default function DynamicHistory(): ReactElement {
  ...
  return (
    <div className="flex flex-col flex-nowrap flex-grow p-4">
      ...
      <h1 className="text-xl text-lightcolor mt-4">Dynamic Proofs</h1>
  ...
  return (
    <div
      className={classNames(
        "flex flex-row flex-nowrap p-2 gap-1 group w-full space-y-2 items-start shadow-xl bg-black/10 backdrop-blur-md rounded-md border-[1px] border-transparent hover:border-secondary hover:bg-black/25 transition-all duration-500 ease-in-out cursor-pointer overflow-hidden",
        props.className
      )}
    >
      ...
      <div className="flex flex-col flex-nowrap flex-grow flex-shrink w-0">
        <div className="flex flex-row items-center text-xs">
          <div className="bg-black/25 backdrop-blur-md text-lightcolor px-1 py-0.5 rounded-sm">
            {request?.method}
          </div>
          <div className="text-lightcolor font-bold px-2 py-1 rounded-md overflow-hidden text-ellipsis">
            {requestUrl?.pathname}
          </div>
        </div>
        <div className="flex flex-row">
          <div className="font-bold text-graycolor">Host:</div>
          <div className="ml-2 text-lightcolor">{requestUrl?.host}</div>
        </div>
        <div className="flex flex-row">
          <div className="font-bold text-graycolor">Notary API:</div>
          <div className="ml-2 text-lightcolor">{request?.notaryUrl}</div>
        </div>
        <div className="flex flex-row">
          <div className="font-bold text-graycolor">TLS Proxy API: </div>
          <div className="ml-2 text-lightcolor">
      ...
      <div className="flex flex-col gap-1">
        {status === "success" && (
          <>
            ...
            /> */}
            <ActionButton
              className="bg-black/10 backdrop-blur-md text-graycolor hover:bg-black/50 hover:text-slate-500"
              onClick={() =>
        ...
        {status === "pending" && (
          <button className="flex flex-row flex-grow-0 gap-2 self-end items-center justify-end px-2 py-1 bg-black/10 backdrop-blur-md text-graycolor font-bold">
            {/* <Icon className="animate-spin" fa="fa-solid fa-spinner" size={1} /> */}
            <span className="text-xs font-bold">Pending</span>
          </button>
        )}
        <ActionButton
          className="flex flex-row flex-grow-0 gap-2 self-end items-center justify-end px-2 py-1 bg-black/10 backdrop-blur-md text-graycolor hover:bg-red-100 hover:text-red-500 hover:font-bold"
          onClick={onDelete}
